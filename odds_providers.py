...
def _normalize(imps):
    s = sum(imps)
    return [x/s for x in imps] if s>0 else [0 for _ in imps]
...
def _consensus_probs(bookmakers: list[dict]) -> dict[str, float]:
    ...
    for name, arr in bucket.items():
        ...
        med = arr_sorted[mid] if len(arr_sorted)%2==1 else 0.5*(arr_sorted[mid-1]+arr_sorted[mid])
        consensus[name] = med
...
def _resolve_sports_to_query(api_key: str, sports_env: str | None) -> list[str]:
    ...
    try:
        ...
        for s in sports:
            if s.get("active", False) and s.get("group","").lower()=="tennis":
                chosen.append(s["key"]); break
        if chosen:
            return chosen[:6]
    except Exception:
        pass
...
def fetch_today_odds_http(sports: list[str] | None = None) -> pd.DataFrame:
    ...
    for sport in sports:
        ...
        if include_links in ("true","false"):
    ...
    cols = ["match_id","sport","league","teams","start_time_iso","market","outcome","book","book_odds","model_prob","note"]
