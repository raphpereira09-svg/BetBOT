...
def _outcome_stats(tagged: list[dict]) -> tuple[dict[str, float], dict[str, int]]:
    # median odds and count per outcome
    bucket = {}
    for oc in tagged:
        name = oc.get("name")
...
def _resolve_sports_to_query(api_key: str, sports_env: str | None) -> list[str]:
    ...
    try:
        ...
        for s in sports:
            if s.get("active", False) and str(s.get("group","")).lower()=="tennis":
                chosen.append(s["key"]); break
        if chosen: return chosen[:6]
    except Exception:
        pass
    return ["upcoming"]
...
def fetch_today_odds_http(sports: list[str] | None = None) -> pd.DataFrame:
    ...
    for sport in sports:
        ...
        for ev in events:
            ...
            # Tag outcomes with their bookmaker
            tagged = []
            for b in bms:
                bkey = b.get("key","")
                for m in b.get("markets", []):
    ...
    cols = ["match_id","sport","league","teams","start_time_iso","market","outcome","book","book_odds",
            "model_prob","consensus_prob","median_odds","n_books","best_vs_median","note"]
