...
def dixon_coles_matrix(lmb_home: float, lmb_away: float, rho: float = 0.12, max_goals: int = 8) -> np.ndarray:
    ...
    M[0,1] *= (1 + lmb_home*rho)
    M[1,0] *= (1 + lmb_away*rho)
    M[1,1] *= (1 - rho)
    M = M / M.sum()
    return M
...
def top_scores(M: np.ndarray, k: int = 3) -> list[tuple[int,int,float]]:
    flat = []
...
def calibrate_lambdas(p_home: float, p_draw: float, p_away: float, rho: float = 0.12) -> tuple[float, float, float]:
    best = (1.4, 1.1, 1e9)
...
def summarize_match(home: str, away: str, p_home: float, p_draw: float, p_away: float, rho: float=0.12, totals_lines: list[float]|None=None):
    lh, la, loss = calibrate_lambdas(p_home, p_draw, p_away, rho=rho)
    M = dixon_coles_matrix(lh, la, rho=rho, max_goals=8)
    H, D, A = hda_from_matrix(M)
    lines = totals_lines or [2.5]
    totals = {float(L): over_prob(M, L) for L in lines}
    mean_home = sum(i * M[i,:].sum() for i in range(M.shape[0]))
    mean_away = sum(j * M[:,j].sum() for j in range(M.shape[1]))
    return {"home": home, "away": away, "lambda_home": lh, "lambda_away": la, "loss": loss, "p_home": H, "p_draw": D, "p_away": A, "mean_home": float(mean_home), "mean_away": float(mean_away), "top_scores": top_scores(M, k=3), "totals_over": totals}
