...
def get_team_signal(signal_table: pd.DataFrame, sport: str, team: str) -> float:
    key = sport.lower().strip() + "|team|" + str(team).strip().lower()
...
def adjust_probabilities(df: pd.DataFrame, signal_table: pd.DataFrame) -> pd.DataFrame:
    ...
    alpha_min = float(os.getenv("ALPHA_MIN", 0.20))
    alpha_max = float(os.getenv("ALPHA_MAX", 0.70))
    alpha_k   = float(os.getenv("ALPHA_K", 0.80))
    for idx, r in out.iterrows():
        ...
        p_adj = inv_logit(z)
        out.at[idx, "adj_prob"] = p_adj
        # Alpha from signal magnitude
        sig_mag = abs(s_home - s_away)
        alpha = max(alpha_min, min(alpha_max, alpha_k * sig_mag))
        out.at[idx, "alpha"] = alpha
        # Blend with market consensus if available
        cons = float(r.get("consensus_prob", r.get("model_prob", base_p)))
        p_blend = (1.0 - alpha) * cons + alpha * p_adj
        out.at[idx, "blend_prob"] = p_blend
    return out
